...
export const EncounterPageComponent = ({ visitId }: EncounterPageProps) => {
  ...
  return (
    <div className="flex-1 lg:p-6 min-h-screen bg-gray-100">
      {/* Patient header card */}
      <div className="mb-0 rounded-t-lg bg-white px-6 py-4 shadow-sm border border-b-0 border-gray-200">
        {isLoadingVisit ? (
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  {patient.sex && (
                    <span className="text-sm text-gray-500">{translateSex(patient.sex)}</span>
                  )}
                  {visitDate && (
                    <>
                      <span className="text-gray-300">·</span>
                      <span className="text-sm text-gray-500">{formatDate(visitDate)}</span>
                    </>
                  )}
                  {patient.nik && (
                    <>
                      <span className="text-gray-300">·</span>
                      <span className="text-sm text-gray-400 font-mono">{patient.nik}</span>
                    </>
                  )}
                </div>
              </div>
      ...
      {/* Tab bar */}
      <div className="flex items-end gap-1 bg-gray-100 px-6 pt-3 border-x border-gray-200">
        {TABS.map((tab) => (
          <FolderTab
            key={tab.id}
            label={tab.label}
      ...
      {/* Tab content */}
      <div className="rounded-b-lg border border-t-0 border-gray-200 bg-white px-6 py-6 shadow-sm">
        {isLoadingVisit ? (
      ...
      {/* Right-side drawers — always available regardless of active tab */}
      <PatientInfoDrawer
